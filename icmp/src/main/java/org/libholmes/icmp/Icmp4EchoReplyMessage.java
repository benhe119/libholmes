...
/** A class to represent an ICMPv4 echo reply message. */
public class Icmp4EchoReplyMessage extends Icmp4Message {
    /** Parse ICMPv4 echo reply message from a generic message
     * @param message the generic message to be parsed
     * @throws ParseException if the message cannot be parsed
     */
    Icmp4EchoReplyMessage(Icmp4Message message) throws ParseException {
        super(message);
        if (getType() != 0) {
            throw new ParseException(
                "invalid message type for ICMPv4 echo reply message");
        }
        if (getCode() != 0) {
            throw new ParseException(
                "invalid message code for ICMPv4 echo reply message");
        }
        if (getBody().length() < 4) {
            throw new ParseException(
                "body too short for ICMPv4 echo reply message");
        }
    }
